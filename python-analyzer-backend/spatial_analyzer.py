...
FAB_PATTERNS = [
    ("PR_HORZ_CIRC_HOOP_TIES", r"#\s*3\s*@\s*8\s*\"?\s*O\.?C\.?\s*TIES", "Pier circular hoop/tie spacing"),
    ("PR_VERT_L_BARS", r"6\s*-\s*#\s*4\s*VERT\.?\s*REBARS", "Pier vertical L bars"),
    ("SW_EW_VERT_REBAR_18_OC", r"#\s*4\s*VERT\.?\s*REBARS?\s*AT\s*18\s*\"?\s*O\.?C\.?") ,
    ("SW_EW_HORZ_CONT_REBAR_12_OC", r"#\s*4\s*CONT\.?\s*REBARS?\s*AT\s*12\s*\"?\s*O\.?C\.?") ,
    ("SW_EW_HORZ_CONT_3_BARS", r"3\s*-\s*#\s*4\s*CONTINUOUS\s*REBAR") ,
    ("SW_EW_REBAR_12_OC", r"#\s*4\s*REBAR\s*AT\s*12\s*\"?\s*O\.?C\.?") ,
...
def extract_fabrication_params(raw_text):
    ...
    if re.search(r"#\s*3\s*@\s*8\s*\"?\s*O\.?C\.?\s*TIES", u):
        add_param(params,"PR_HORZ_CIRC_HOOP_TIE_SPACING","Pier circular hoop/tie spacing",'#3 @ 8" OC TIES','Matched #3 @ 8" OC TIES inside selected region.')
    if re.search(r"6\s*-\s*#\s*4\s*VERT\.?\s*REBARS", u):
        add_param(params,"PR_VERT_L_BARS_TOTAL","Pier vertical L bars",'6-#4 VERT REBARS','Matched 6-#4 VERT REBARS inside selected region.')
    ...
    if re.search(r"#\s*4\s*VERT\.?\s*REBARS?\s*AT\s*18\s*\"?\s*O\.?C\.?", u):
        add_param(params,f"{wall_prefix}_VERT_L_BAR_SPACING",f"{wall_prefix} vertical L bar spacing",'#4 VERT. REBARS AT 18" OC','Matched #4 VERT. REBARS AT 18" OC.')
    if re.search(r"#\s*4\s*CONT\.?\s*REBARS?\s*AT\s*12\s*\"?\s*O\.?C\.?", u):
        add_param(params,f"{wall_prefix}_HORZ_CONT_BAR_SPACING",f"{wall_prefix} horizontal continuous bar spacing",'#4 CONT. REBARS AT 12" OC','Matched #4 CONT. REBARS AT 12" OC.')
    if re.search(r"3\s*-\s*#\s*4\s*CONTINUOUS\s*REBAR", u):
        add_param(params,f"{wall_prefix}_HORZ_CONT_BAR_COUNT",f"{wall_prefix} horizontal continuous bar count",'3-#4 CONTINUOUS REBAR','Matched 3-#4 CONTINUOUS REBAR.')
